...
void s(RIESENI_DATA **linkes_rieseni);
...
void a(HRACI_DATA **linked_hraci){
    ...
    n->next = m;
    tmp->next = n;
    printf("A: Uspesne pridany zaznam na pozíciu %s.\n", n->PID);
}
...
void v3(HRACI_DATA **linked_hraci){
    ...
    while(n != NULL){
        ...
        printf("Krajina: %s\n", n->Krajina);
        printf("RokNar: %s\n", n->RokNar);
        printf("Vysledky: %s / %c / %s / %c / %s / %d\n", n->Riesenia[0]->GID, n->Riesenia[0]->NarSut, n->Riesenia[0]->SID, n->Riesenia[0]->NarHry, n->Riesenia[0]->DatHry, n->Riesenia[0]->Trvanie);
        n = n->next;
        printf("\n");
    }
}
